...
class Mirasvit_Seo_Model_Config
{
    ...
    //Separator for complex filter names in Layered Navigation
    const LN_SEPARATOR_UNDERSCORE                       = '_';
    /*const LN_SEPARATOR_PLUS                             = '+';*/
    const LN_NO_SEPARATOR                               = 0;

    const SEO_POST_INSTALL_MESSAGE = 'seo_post_install_message_enabled';
    const AUTOLINK_POST_INSTALL_MESSAGE = 'autolinks_post_install_message_enabled';
    const SEOSITEMAP_POST_INSTALL_MESSAGE = 'seositemap_post_install_message_enabled';
...
///////////// Extended Settings
    ...
    public function isRemoveKeywordsMetaTag($store)
    {
        return Mage::getStoreConfig('seo/extended/remove_keywords_meta_tag', $store);
    }
...
///////////// Rich Snippets and Opengraph
    ...
    public function getLogoOrganizationSnippets()
    {
        return Mage::getStoreConfig('seo/snippets/logo_organization_snippets');
    }

    public function getManualLogoOrganizationSnippets()
    {
        return trim(Mage::getStoreConfig('seo/snippets/manual_logo_organization_snippets'));
    }

    public function getSameAs($store = null)
    {
        $conf = Mage::getStoreConfig('seo/snippets/same_as_organization_snippets', $store);
        $links = explode("\n", trim($conf));
    ...
    public function getIsTwitterCard($store = null)
    {
        return Mage::getStoreConfig('seo/snippets/is_twitter_card');
    }

    public function getTwitterUsername($store = null)
    {
        return Mage::getStoreConfig('seo/snippets/twitter_username');
    }
...
///////////// SEO URL
    ...
    {
        return Mage::getStoreConfig('seo/url/layered_navigation_friendly_urls_separator');
    }
