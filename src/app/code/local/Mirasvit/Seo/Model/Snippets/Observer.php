...
class Mirasvit_Seo_Model_Snippets_Observer extends Varien_Object {
    ...
    public function productFilter($html, $product) {
         ...
         * Varien_Object
         */
        $currentSeoObject = Mage::helper('seo')->getCurrentSeo();

        $replacement = '';

        if (strpos($html, 'product-name') !== false) {
            $html = preg_replace('/\\"product\\-name(.*?)\\"/i','"product-name $1" itemprop="name"',$html,1);
        } elseif ($name = $currentSeoObject->getTitle()) {
            $name = '<meta itemprop="name" content="'.$name.'"/>';
            $replacement .= $name;
        }

        /*need such construction to take most common variants into account*/
        if (strpos($html, 'short-description') !== false) {
            $html = preg_replace('/\\"short\\-description\\"/i','"short-description" itemprop="description"',$html,1);
        } else {
            $shortDescriptionArray = array();
            $shortDescriptionArray[] = $currentSeoObject->getShortDescription();
            $shortDescriptionArray[] = $this->_getLoadedProduct($product)->getShortDescription();
            $shortDescriptionArray[] = $currentSeoObject->getFullDescription();
            $shortDescriptionArray[] = $this->_getLoadedProduct($product)->getDescription();
            $shortDescriptionArray[] = $currentSeoObject->getMetaDescription();

            $shortDescriptionArray = array_diff($shortDescriptionArray, array(0, null, '', ' '));

            foreach ($shortDescriptionArray as $shortDescription) {
                if ($shortDescription == strip_tags($shortDescription)) {
                    $description = '<meta itemprop="description" content="'.$shortDescription.'"/>';
        ...
        if ($this->getConfig()->isEnabledRichSnippetsDimensions()) {
            ...
            if ($dimensionalUnit) {
                $dimensionalUnit = Mage::helper('seo/snippets_dimensions')->prepareDimensionCode($dimensionalUnit);
            }
    ...
    public function offerFilter($html, $product)
    {
        ...
        /* Google Structured Data Testing Tool throws Warning on price attribute with comma */
        $productFinalPrice = Mage::helper('seo/snippets_price')->formatPriceValue($productFinalPrice);
    ...
    /**
     * Load product to get needed attributes
     * @param Mage_Catalog_Model_Product $product
     * @return Mage_Catalog_Model_Product
     */
    protected function _getLoadedProduct($product) {
        if ($product) {
