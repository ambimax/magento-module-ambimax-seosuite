...
class Mirasvit_Seo_Model_Observer extends Varien_Object
{
    ...
    /**
     * Fix for correct trailing slash redirecting of a Home page,
     * but can cauuse redirect loop for some stores, so is disabled by default
     */
    protected $_forceTrailingSlashForHome = false;

    protected static $_seo = null;

    /**
     * If set to "true" - different algorithm will be used
     * to calculate the longest relevant category chain in
     * product URL
     */
    protected static $_mixedCategoryId = false;

    public function __construct()
    {
        $this->_urlHelper = Mage::helper('seo/url');
        ...
        switch ($params['attribute']) {
            ...
            case 'description':
                ...
                if (count($state->getFilters()) > 0) {
                    $outputHtml = '';
                } elseif($categoryDescription = self::$_seo->getCategoryDescription()) {
                    $outputHtml = $categoryDescription;
                }
                break;
        }
    ...
    public function checkUrl($e)
    {
        ...
        $this->_redirectFromRedirectManagerUrlList();
        $this->_redirectHelper->unsetFlag();

        $this->_redirectFromOldLayeredNavigationUrl();

        $urlToRedirect = $this->getUrlWithCorrectEndSlash($url);

        if ($this->_forceTrailingSlashForHome) {
            if ($url == '/' && $urlToRedirect == '/' && $url != $fullUrl) {
                $this->redirect($this->getUrlWithCorrectEndSlash($this->_urlHelper->getCurrentBaseUrl()));
            }
        }

        if ($url != $urlToRedirect) {
            $this->redirect(rtrim($this->_urlHelper->getCurrentBaseUrl(), '/') . $urlToRedirect);
        }
    ...
    protected function _prepareRedirectUrl($redirectUrl) {
        if (stripos($redirectUrl, 'http://') === false
                && stripos($redirectUrl, 'https://') === false) {
                    return $this->_urlHelper->getCurrentBaseUrl() . ltrim($redirectUrl, '/');
        }
     ...
     * @return bool
     */
    protected function _redirectFromRedirectManagerUrlList() {
        ...
        if (Mage::getVersion() > '1.4.1.1') {
            $url = str_replace($this->_urlHelper->getCurrentBaseUrl(), '', $currentUrl);
            $trimmedUrl = str_replace(rtrim($this->_urlHelper->getCurrentBaseUrl(), '/'), '', $currentUrl);
            /*code needed to check redirects from pages with and without store code added to URL*/
            if (($currentUrl == $url) && ($url == $trimmedUrl)) {
                $storeCode = Mage::app()->getStore()->getCode();
                $currentBaseUrl = strpos($this->_urlHelper->getCurrentBaseUrl(), $storeCode . '/') == true ?
                str_replace($storeCode .'/', '', $this->_urlHelper->getCurrentBaseUrl()) :
                str_replace($storeCode, '', $this->_urlHelper->getCurrentBaseUrl());
                $url = str_replace($currentBaseUrl, '', $currentUrl);
                $trimmedUrl = str_replace(rtrim($currentBaseUrl, '/'), '', $currentUrl);
            }
            /*end of check*/
            ...
            $redirectCollection->getSelectSql()->where(new Zend_Db_Expr($where), null, Varien_Db_Select::TYPE_CONDITION);
        } else { //compatibility with older Magento versions
            $redirectCollection->getSelect()
                ->where('url_from = ' . "'" . addslashes($currentUrl) . "'"
                    . ' OR ' . 'url_from = ' . "'" . addslashes(str_replace($this->_urlHelper->getCurrentBaseUrl(), '', $currentUrl)) . "'"
                    . ' OR ' . 'url_from = ' . "'" . addslashes(str_replace(rtrim($this->_urlHelper->getCurrentBaseUrl(), '/'), '', $currentUrl)) . "'"
                    . ' OR ' . "'" . addslashes($currentUrl) . "'" . " LIKE CONCAT(REPLACE(url_from, '*', '%'))");
        }

        $redirectCollection->getSelect()->order('sort_order ASC');
        ...
        foreach ($redirectCollection as $redirect) {
            ...
            if ($this->_redirectHelper->_checkRedirectPattern($redirect->getUrlFrom(), $redirect->getUrlTo(), $action)) {
                continue;
            }

            if ($this->_redirectHelper->checkForLoop($urlTo)) {
                continue;
            }

            if ($currentUrl == $urlFrom
                || (stripos($redirect->getUrlFrom(), '*') !== false
                    && $this->_redirectHelper->_checkRedirectPattern($redirect->getUrlFrom(), $currentUrl)) ) {
                        $this->_redirectHelper->setFlag($currentUrl); //mark URL to prevent looping
    ...
    public function checkProductUrlRedirect($e)
    {
        ...
        //need to not alter product URLs with tracking parameters
        $currentUrl = Mage::helper('core/url')->getCurrentUrl();
        if(preg_match("/fs_.*/", $currentUrl) || preg_match("/utm_.*/", $currentUrl) || preg_match("/gclid=.*/", $currentUrl)) {
            return;
        }
        ...
        $productUrl = str_replace($this->_urlHelper->getCurrentBaseUrl(), '', $product->getProductUrl());
   ...
   /**
    ...
    protected function _addUrlRewrite($collection)
    {
        ...
        if (!$urlRewrites) {
            ...
            if (Mage::helper('mstcore/version')->getEdition() != 'ee') { //we don't use Mirasvit_Seo_Model_Config::URL_FORMAT_LONG for EE
                ...
                $urlRewrites = array();
                $fetchedRewrites = array();
                $fetchedRewrites = $connection->fetchAll($select);

                // Needed for "Long" product URLs to work correctly if /root/category/childcategory/subcategory
                // has smaller id than /root/category/childcategory
                if ($urlFormat == Mirasvit_Seo_Model_Config::URL_FORMAT_LONG && self::$_mixedCategoryId === true) {
                    for ($i=0; $i < count($fetchedRewrites); $i++) {
                        $fetchedRewrites[$i]["order"] = count(explode('/', $fetchedRewrites[$i]['request_path']));
                    }
                    $order = array();
                    foreach ($fetchedRewrites as $key => $row)
                    {
                        $order[$key] = $row['order'];
                    }
                    array_multisort($order, SORT_DESC, $fetchedRewrites);
                }

                foreach ($fetchedRewrites as $row) {
    ...
    //if we use "Product URL Key Template" and "URL Key" = NULL we will create  URL Key by Template for Magento Enterprise
    public function updateUrlKeyByTemplateIfEmpty($observer)
    {
