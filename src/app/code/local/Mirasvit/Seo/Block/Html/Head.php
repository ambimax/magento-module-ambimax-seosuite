...
} elseif (Mage::helper('core')->isModuleEnabled('ShopInDev_SuperMinify') && class_exists('ShopInDev_SuperMinify_Block_Page_Html_Head')) {
    abstract class Mirasvit_Seo_Block_Html_Head_Abstract extends ShopInDev_SuperMinify_Block_Page_Html_Head {
...
} elseif (Mage::helper('core')->isModuleEnabled('EM_Em0126settings') && class_exists('EM_Em0126settings_Block_Html_Head')) {
    abstract class Mirasvit_Seo_Block_Html_Head_Abstract extends EM_Em0126settings_Block_Html_Head {

    }
} elseif (Mage::helper('core')->isModuleEnabled('Smartwave_Mango') && class_exists('Smartwave_Mango_Block_Html_Head')) {
    abstract class Mirasvit_Seo_Block_Html_Head_Abstract extends Smartwave_Mango_Block_Html_Head {

    }
} elseif (Mage::helper('core')->isModuleEnabled('Magenmagic_Smartwave') && class_exists('Magenmagic_Smartwave_Block_Porto_Html_Head')) {
    abstract class Mirasvit_Seo_Block_Html_Head_Abstract extends Magenmagic_Smartwave_Block_Porto_Html_Head {

    }
} elseif (Mage::helper('core')->isModuleEnabled('Sm_Furnicom') && class_exists('Sm_Furnicom_Block_Page_Html_Head')) {
    abstract class Mirasvit_Seo_Block_Html_Head_Abstract extends Sm_Furnicom_Block_Page_Html_Head {

    }
} elseif (Mage::helper('core')->isModuleEnabled('Sm_Market') && class_exists('Sm_Market_Block_Page_Html_Head')) {
    abstract class Mirasvit_Seo_Block_Html_Head_Abstract extends Sm_Market_Block_Page_Html_Head {

    }
} elseif (Mage::helper('core')->isModuleEnabled('Ves_Tempcp') && class_exists('Ves_Tempcp_Block_Html_Head')) {
    abstract class Mirasvit_Seo_Block_Html_Head_Abstract extends Ves_Tempcp_Block_Html_Head {

    }
} elseif (Mage::helper('core')->isModuleEnabled('Bubble_Queryfier') && class_exists('Bubble_Queryfier_Block_Page_Html_Head')) {
    abstract class Mirasvit_Seo_Block_Html_Head_Abstract extends Bubble_Queryfier_Block_Page_Html_Head {

    }
} elseif (Mage::helper('core')->isModuleEnabled('TM_CDN') && class_exists('TM_CDN_Block_Html_Head')) {
    abstract class Mirasvit_Seo_Block_Html_Head_Abstract extends TM_CDN_Block_Html_Head {
...
class Mirasvit_Seo_Block_Html_Head extends Mirasvit_Seo_Block_Html_Head_Abstract
{
    ...
    protected function _construct()
    {
        parent::_construct();
        /*
         * to avoid adding canonical and alternate links several times
         * when Head block class is called multiple times during the page load
         */
        if (!Mage::registry('m_LinksSet')) {
            $this->setupCanonicalUrl();
            $this->setupAlternateTag();
            Mage::register('m_linksSet', 1);
        }
    }
    ...
    public function setupCanonicalUrl() {
        ...
        if ($canonicalUrl = Mage::helper('seo/canonical')->getCanonicalUrl()) {
                    ...
                    if (!$isAlternateAdded) {
                        ...
                        foreach ($storesArray as $store)
                        {
                            ...
                            if (Mage::app()->getRequest()->getModuleName() == 'cms'
                                && Mage::app()->getRequest()->getActionName() != 'noRoute') {
                                    $cmsStoresIds = Mage::getSingleton('cms/page')->getStoreId();
                                    if ((is_array($cmsStoresIds) && in_array($store->getId(), $cmsStoresIds))
                            ...
                            if (Mage::app()->getRequest()->getControllerName() == 'product') {
                                ...
                                if ($isMagentoEe && $product->isVisibleInSiteVisibility()) {
                                    $url = $store->getBaseUrl() . $this->getEeAlternateProductUrl() . $urlAddition;
                                    $addLinkRel = true;
                                } elseif ($product->isVisibleInSiteVisibility()) {
                                    $url = $store->getBaseUrl() . $this->getAlternateProductUrl($product->getId(), $categoryId, $store->getId()) . $urlAddition;
                                    $addLinkRel = true;
                                }
                                // $addLinkRel = true;
                            }
                            if ($blogAlternate = $this->addAWBlogAlternates($store)) { //Adding alternates to AW Blog pages
                                $url = $blogAlternate;
                                $addLinkRel = true;
                            }
                            ...
                            if (!Mage::getStoreConfig('seo/general/is_alternate_hreflang', $store->getStoreId())) {
                                $addLinkRel = false;
                            }
    ...
    public function getAlternateProductUrl($productId, $categoryId, $storeId)
    {
        ...
        return $urlRewriteObject->getRequestPath() ? $urlRewriteObject->getRequestPath() : false;
    }
    ...
    /**
     * Provides alternate URL for AW Blog page
     *
     * @return string
     */
    protected function addAWBlogAlternates($store) {
        if (!Mage::helper('mstcore')->isModuleInstalled('AW_Blog')) {
            return false;
        }
        $route = Mage::getStoreConfig('blog/blog/route', $store->getId());
        $route = $route ? $route : "blog";
        if (Mage::helper('seo')->getFullActionCode() == 'blog_post_view') {
            $identifier = Mage::getSingleton('blog/post')->getIdentifier();
            if (substr(Mage::helper('core/url')->getCurrentUrl(), -1) == '/') {
                $identifier .= '/';
            }
            return $url = $store->getbaseUrl() . $route . '/' . $identifier;
        } elseif (Mage::helper('seo')->getFullActionCode() == 'blog_index_list') {
            return $url = $store->getbaseUrl() . $route;
        }
    }
}
