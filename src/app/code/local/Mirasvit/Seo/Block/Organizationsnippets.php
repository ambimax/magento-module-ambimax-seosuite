...
class Mirasvit_Seo_Block_Organizationsnippets extends Mage_Core_Block_Template
{
    ...
    public function getLogoUrl() {
        if ($this->_config->getLogoOrganizationSnippets()) {
            $logo = $this->_config->getManualLogoOrganizationSnippets();
        } else {
            $logo = Mage::getDesign()->getSkinUrl() . Mage::getStoreConfig('design/header/logo_src', Mage::app()->getStore()->getStoreId());
        }

        return $logo;
    }

    public function getSameAs($isJson = false) {
        $links = $this->_config->getSameAs();
        $linksRow = '';
        ...
        if ($links) {
            if ($isJson) {
                ...
                foreach ($links as $link) {
                    $linksRow .= $this->getSnippetRow("sameAs", $link, $isJson);
                }
            }
