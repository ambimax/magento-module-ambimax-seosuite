...
class Mirasvit_SeoAutolink_Model_Observer extends Varien_Object
{
    ...
    /**
     * Add links inside block
     *
     * @param object Varien_Event_Observer
     * @return void
     */
    public function addBlockOutputHelper($e)
    {
        ...
        if (!in_array($template, $templatePaths)) {
            return;
        }

        if ($this->isTemplateIgnored($template)) {
            return;
        }
    ...
    /**
     * Check if template is ignored
     *
     * @param string $templatePath
     * @return bool
     */
    protected function isTemplateIgnored($templatePaths) {
        ...
        foreach ($ignoredTemplates as $template) {
            if (strpos($templatePaths, $template) !== false) {
                return true;
            }
        }
