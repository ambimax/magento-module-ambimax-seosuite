...
class Mirasvit_SeoAutolink_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    protected static $_maxLinkPerPage = 0;
    protected static $_isEnabledLanguageWithoutSpaceCompatibility = false;


    public function __construct()
    {
        $maxLinkPerPage = $this->getMaxLinkPerPage();
        self::$_maxLinkPerPage = $maxLinkPerPage ? $maxLinkPerPage : self::MAX_NUMBER;
        self::$_isEnabledLanguageWithoutSpaceCompatibility = $this->getConfig()->isEnabledLanguageWithoutSpaceCompatibility(Mage::app()->getStore()->getStoreId());
    }
    ...
    public function getLinks($text)
    {
        ...
        $textArrayWithMaxSymbols = (self::$_isEnabledLanguageWithoutSpaceCompatibility) ? array($text) : $this->splitText($text); //return array
        $where = array();
    ...
    {
        // To ensure that links are added only on the frontend
    ...
    public function _addLinks($text, $links, $replacementCountForTests = false)
    {
        ...
        foreach ($links  as $link) {
            $selfLinkedFull = $link->getUrl();
            $selfLinkedShort = $this->toSingleSlashes(Mage::getBaseUrl().$selfLinkedFull);
            if ($this->getConfig()->getSkipRecursive()
                && ($selfLinkedFull == $currentUrl || $selfLinkedShort == $currentUrl)) {
                continue; // skipping links pointing to current page if configured
            }
    ...
    {
        if ($this->currentNumberOfLinks >= self::$_maxLinkPerPage) { // we use it also in function addLinksToSource for performance improvement
            return $source;
        }
    ...
    {
        if ($this->currentNumberOfLinks >= self::$_maxLinkPerPage) { // we use it also in function replace for performance improvement
            return $source;
        }
        ...
        foreach ($explodeSource as $keySource => $valSource) {
            ...
            if (!$isStopReplacement && ($size < $sizeExplodeSource)
                ...
                && (((!empty($explodeSource[$keySource + 1][0]))
                    && (in_array($explodeSource[$keySource + 1][0], $nextSymbol) || self::$_isEnabledLanguageWithoutSpaceCompatibility))
                ...
                && ((empty($explodeSource[$keySource][strlen($explodeSource[$keySource]) - 1]))
                    ...
                    || ((!empty($explodeSource[$keySource][strlen($explodeSource[$keySource]) - 1]))
                        && (in_array($explodeSource[$keySource][strlen($explodeSource[$keySource]) - 1], $prevSymbol) || self::$_isEnabledLanguageWithoutSpaceCompatibility)))
                ...
                $numberOfReplacements++;
                  ...
                  && ($direct == 1)
                  && (((!empty($explodeSource[$keySource][strlen($explodeSource[$keySource]) - 1]))
                      && (in_array($explodeSource[$keySource][strlen($explodeSource[$keySource]) - 1], $nextSymbol) || self::$_isEnabledLanguageWithoutSpaceCompatibility))
                          || (preg_match($prevPattern, $explodeSource[$keySource]))
                          || (empty($explodeSource[$keySource][strlen($explodeSource[$keySource]) - 1])))
                  && ((empty($explodeSource[$keySource + 1][0]))
                      || (preg_match($nextPattern, $explodeSource[$keySource + 1]))
                      || ((!empty($explodeSource[$keySource + 1][0]))
                        && (in_array($explodeSource[$keySource + 1][0], $prevSymbol) || self::$_isEnabledLanguageWithoutSpaceCompatibility)))
                  && ($this->_sizeExplode < $maxReplacements)
                  && !$replaceNumberOne) {
                $prepareSourse .= $valSource.$replace;
                $this->_sizeExplode++;
            ...
            if ($this->currentNumberOfLinks +  $numberOfReplacements == self::$_maxLinkPerPage) {
    ...
    */
    ...
    protected function getDebugMessage()
    {
        ...
        $html[] = "Links limit per page: ". (self::$_maxLinkPerPage ? self::$_maxLinkPerPage : "unlimited")."</br>";
//        $html[] = "Unique links in block: ".count($this->linksInBlock)."<br>";
    ...
    /**
     * Replaces double slashes in a given URL:
     * http://somesite.com//home-decor/electronics/madison-rx3400.html
     * to
     * http://somesite.com/home-decor/electronics/madison-rx3400.html
     *
     * @param string $link
     * @return string
     */
    public function toSingleSlashes($link)
    {
