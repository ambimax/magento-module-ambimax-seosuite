...
class Mirasvit_SeoFilter_Helper_Data extends Mage_Core_Helper_Abstract
{
   /**
	...
	{
		$separator = Mage::getSingleton('seo/config')->getFilterSeparator();
	    ...
	    $string = str_replace(array('-'), '', $string); //replacing system URL separators
	    $string = $s == '_' ? str_replace('__', '_', $string) : $string; // prevents "Bed & Bath" from becoming bed__bath
	    return $string;
	}
}