...
class Mirasvit_SeoSitemap_Model_Sitemap extends Mirasvit_SeoSitemap_Model_Sitemap_Abstract
{
    ...
    protected $date;
    protected $baseUrl;
    protected $isFrontSecure;
    protected $isAdminSecure;
    protected $isSeoEnabled;
    protected $seoImageHelper;
    protected $isEnableImageAlt;
    ...
    public function init($storeId) {
        ...
        $this->isFrontSecure = Mage::app()->getStore($storeId)->isFrontUrlSecure();
        $this->isAdminSecure = Mage::app()->getStore()->isAdminUrlSecure();
        if ($this->isSeoEnabled = Mage::helper('core')->isModuleEnabled('Mirasvit_Seo')) {
            $this->seoImageHelper = Mage::helper('seo/rewrite_image');
            $this->isEnableImageAlt = Mage::getSingleton('seo/config')->getIsEnableImageAlt();
        }
    }
    ...
    public function getImageUrl($file, $item, $storeId)
    {
        ...
        if ($this->isAdminSecure && !$this->isFrontSecure) {
     ...
     * @return array
     */
    protected function deleteWrongSymbols($text) {
        $search  = array(chr(2));
        $replace = '';
                    ...
                    if (is_array($gallery)) {
                        foreach ($gallery as $image) {
                            ...
                            try {
                                ...
                                if ($imageUrl = $this->getImageUrl($image['file'], $item,  $this->storeId)) {
                                    $imageTitle = $this->isSeoEnabled && $this->isEnableImageAlt ?
                                                  $this->seoImageHelper->getTitle($item) :
                                                  $item->getImageLabel();
                                    $imageTitle = htmlspecialchars($imageTitle);
                                    $imageTitle = $this->deleteWrongSymbols($imageTitle);
                                    $imageCaption = $imageTitle ?
                                                    '<image:title>'.$imageTitle.'</image:title>'.'<image:caption>'.$imageTitle.'</image:caption>' :
                                                    '';
                                    $images .= '<image:image><image:loc>' . htmlspecialchars($imageUrl) . '</image:loc>'.$imageCaption.'</image:image>';
                                }
    ...
    public function generateAWblogSitemap() {
        if (Mage::helper('mstcore')->isModuleInstalled('AW_Blog')) {
            ...
            $collection->getSelect()->distinct(true); // Codazon_Blogfeatures_Model_Blog compatibility
